...
\end{block}
\vspace{0.2em}
\emp{Honest result (next slides):} on this \textbf{deterministic} toy there is no
maximisation bias to remove, so across 5 seeds vanilla and Double-Q come out
\emp{statistically tied} ($83.3$ vs $83.1$, overlapping bands). The variant earns its
keep under noise, not here. Reporting both, with error bars, is the point.
\end{frame}
...
\scriptsize \emp{Episode return climbs from $\sim$40 to $\sim$85 and converges. The band
...
\footnotesize
\centering
\includegraphics[width=0.60\linewidth]{\fig{13_baseline_comparison.png}}
\vspace{0.1em}
\raggedright
\begin{itemize}\setlength\itemsep{0pt}
\item Q-learning (\good{83.3 $\pm$ 3.8}) beats every hand rule by a wide margin: the best,
      \code{random}, reaches only $\sim$35; the structured rules go negative.
\item Double-Q (\warn{83.1 $\pm$ 3.7}) is \emp{statistically tied} with vanilla: no
      maximisation bias to remove on a deterministic graph, so it neither helps nor hurts.
\item \emp{Takeaway:} the learned timing dominates every hand rule across all 5 seeds.
\end{itemize}
\end{frame}
...
\raggedright
\begin{itemize}\setlength\itemsep{1pt}
\item Both profiles beat baseline (means over 5 seeds): \good{+20\% to +39\%} (art),
      \good{+36\% to +38\%} (tourist).
...
\raggedright
\begin{itemize}\setlength\itemsep{1pt}
\item \emp{MaskablePPO} is the clear winner: stable across seeds, finds book-early, best
      or tied-best at every crowd.
\item \emp{Masking is decisive where book-early matters:} at the packed tourist, unmasked
      ...
      MaskablePPO holds \good{3685}, on every seed.
\item \emp{DQN} is crowd-fragile and high-variance on the art lover: its baseline falls
      $4527\!\to\!2044$ as the crowd grows, with seed std up to $\pm$2{,}600 (the wide error bars).
\end{itemize}
\end{frame}
...
\item \emp{RL on top (the optimal visit).} Given that intervened world, the optimal
      ...
      \good{+20\% to +39\%} (art) and \good{+36\% to +38\%} (tourist), averaged over 5 seeds.
\end{enumerate}
\vspace{0.4em}
\begin{center}
...
\item \emp{Five-seed algorithm matrix.} The 30-cell comparison is now averaged over 5
      training seeds (mean $\pm$ std). This confirms DQN's high variance on the art lover,
      and exposes that the art-lover / max-crowd MaskablePPO gain is itself high-variance:
      4 of 5 seeds reach $\sim$+41\%, one collapses, so that cell averages \warn{+20\%} with a wide band.
