...
\begin{description}[leftmargin=0pt,style=nextline,itemsep=3pt]
\item[Where you are in the booking phase \textmd{(slots 1 to 3: $1,\ 0,\ 1.0$)}] Three numbers that
locate the agent in the four-step process just described. \emph{Slot 1} is a phase flag: $1$ while on
the booking screen, $0$ once walking the museum (here $1$, still booking). \emph{Slot 2} says which of
the four decisions is being made now, written as a fraction: $0$ for the lead-time decision, then
$0.25$, $0.5$, $0.75$ for the three masterpieces in turn (here $0$, so the lead-time decision).
\emph{Slot 3} is the lead time picked so far as a fraction of the $35$-day maximum: it begins at $1.0$
(the full month, the default before any choice) and would drop if the agent picked a shorter lead, for
instance $7$ days shows as $7/35=0.2$ (here $1.0$, nothing chosen yet).
\item[The booking screen, how full each lead time is \textmd{(slots 4 to 7: $0.12,\ 0.38,\ 0.62,\ 1.0$)}]
Four numbers, one per lead time, giving the \emph{fraction of masterpiece time-slots still free} if you
reserve that far ahead: slot 4 is booking $1$ day before, slot 5 is $7$ days before, slot 6 is $21$
days, slot 7 is $35$ days. They climb from left to right because slots fill up as the date nears. On
this busy day only $12\%$ are still free if you wait until the day before, against $38\%$ a week ahead,
$62\%$ three weeks ahead, $100\%$ a month ahead. This is the \emph{fill curve}; it is the whole reason
the agent learns to reserve further ahead when the museum is busier, since leaving it late on a packed
day means the good windows are already gone.
...
\item[Windows you currently hold \textmd{(slots 9 to 11: $0,\ 0,\ 0$)}] Once the bookings are
committed, each masterpiece has a reserved \emph{window}, the time-slot you are allowed to enter. These
three numbers give the start of that window for Botticelli, Leonardo and Raphael, as a fraction of the
day ($0.5$ would be midday), or $0$ if that masterpiece has no window yet. All $0$ here because the
snapshot is the opening screen, before anything is booked.
\item[Check-ins done \textmd{(slots 12 to 14: $0,\ 0,\ 0$)}] One flag per masterpiece, in the same
order: $1$ once the visitor has actually stepped into that room \emph{during} its reserved window and
collected the reward (a ``check-in''), $0$ until then. All $0$ before the visit has begun.
\item[Access read of the room you are in \textmd{(slots 15 to 18: $0,\ 0,\ 0,\ 0$)}] Four numbers
describing the \emph{current} room as a reservation: (15) is it one of the gated masterpiece rooms at
all; (16) is it a masterpiece you did \emph{not} book, so you cannot get in; (17) may you appreciate it
right now, because your window is open or you have already checked in; (18) how long until your window
opens, as a fraction of the day. All four are $0$ whenever you are not in a masterpiece room, as here
in \texttt{A1}; they come alive only inside a gated room, letting the agent connect ``I am getting
nothing here'' to ``my reservation has not opened yet''.
\item[Frozen pace estimate \textmd{(slots 19 to 21: $0.23,\ 0.57,\ 0.66$)}] During earlier training the
agent measured \emph{when in the day it tends to reach} each masterpiece and kept a running average,
then froze it. Here it expects to reach Botticelli about a quarter into the day ($0.23$), Leonardo just
past the half ($0.57$), Raphael soon after ($0.66$), each as a fraction of the day. Being frozen (not
updated mid-episode), they act as a stable estimate of the visitor's pace; the committed windows are
placed near these times so the visitor can actually reach each slot rather than miss it.
\item[The room-level tail \textmd{(slots 22 to 25: $0,\ 0.22,\ 0.05,\ 0.99$)}] The last four are
exactly the room-level signals from the as-is state: how drained the current room is, its value
(importance over $10$), its crowd and the egress slack to the exit. They let the one network handle the
walking part of the episode, reusing what it already learned on the as-is task.
\end{description}
...
\end{center}
Only two numbers moved from the opening screen: the step is now $0.25$ (the first masterpiece, no longer
the lead) and the lead-so-far has dropped to $0.6$ ($=21/35$). Nothing is booked yet, so every other
block is unchanged.
...
\end{center}
The phase flag is now $0$ (walking) and the booking step is full ($1$). The three windows now show their
start times ($0.22$, $0.61$, $0.65$ of the day) and the Botticelli check-in flag has flipped to $1$. The
access read lights up: this is a gated masterpiece ($1$), it is not unbooked ($0$), you may appreciate
it now ($1$) and the time-to-open is slightly negative ($-0.18$, the window has just opened). The room
tail is the punchline: value $1.0$ (Botticelli is importance $10$, the maximum) and crowd $1.0$ (packed
to capacity), but because you are inside your reserved window you collect the reward regardless of the
crush.
...
\end{center}
The time of day is now $0.94$, almost closing; all three check-in flags read $1$: every masterpiece is
done. The access read is back to all zeros, because Caravaggio is not a gated room, you walk straight
in; its value is still high ($0.74$) while its crowd is light ($0.13$). The number that matters now is
the egress slack, down to $0.05$: there is barely time to reach the exit, which is exactly why the agent
is on the first floor on its way out.
