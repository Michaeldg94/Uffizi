...
def main() -> None:
    ...
    if q_data is not None:
        ...
        # Baseline comparison: 5 baselines plus vanilla Q-learning plus
        # Double Q-learning (the second member of the Q-learning family
        # added in this version of the pipeline).
        baselines = q_data.get("baseline_results", [])
        q_return = q_data.get("q_learning_eval", {}).get("mean_return", 0.0)
        dq_eval = q_data.get("double_q_learning_eval")
        dq_return = dq_eval.get("mean_return") if isinstance(dq_eval, dict) else None
        if baselines:
            _saved("02_baseline_comparison",
                   plot_baseline_comparison(baselines, q_return,
                                             str(figs / "02_baseline_comparison.png"),
                                             double_q_return=dq_return))
